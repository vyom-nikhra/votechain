...
export const walletConnectedMiddleware = (req, res, next) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: 'Authentication required'
  ...
  if (!req.user.walletAddress) {
    return res.status(403).json({
      success: false,
      message: 'Wallet connection required to access this resource'
    });
  }
...
export const kycVerifiedMiddleware = (req, res, next) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: 'Authentication required'
